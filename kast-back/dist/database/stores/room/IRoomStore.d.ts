...
export default abstract class IRoomStore {
    ...
    abstract findParticipant(roomId: string, userId: string): Promise<Participant | undefined>;
    ...
    abstract updateParticipantMedia(type: 'audio' | 'video' | 'screenShare', roomId: string, clientId: string, userId: string, renewParticipantMedia: ParticipantTrackOptions): Promise<boolean>;
    abstract updateEmptyParticipant(roomId: string, clientId: string, userId: string): Promise<void>;
    abstract getInviteLink(roomId: string): Promise<string>;
    abstract updateRaiseHand(roomId: string, userId: string, raiseHand: boolean): Promise<boolean>;
    ...
    abstract mute(action: MuteAction, source: MuteSource, userId: string, roomId: string): Promise<boolean>;
    abstract muteAll(action: MuteAction, userId: string, roomId: string): Promise<boolean>;
    abstract muteAudio(action: MuteAction, source: MuteSource, userId: string, roomId: string): Promise<boolean>;
    abstract muteVideo(action: MuteAction, source: MuteSource, userId: string, roomId: string): Promise<boolean>;
    abstract muteScreen(action: MuteAction, source: MuteSource, userId: string, roomId: string): Promise<boolean>;
    abstract kick(roomOwnerUserId: string, userId: string, roomId: string): Promise<boolean>;
    abstract playPauseMedia(media: PlayingType, status: boolean, roomId: string, userId: string): Promise<void>;
}
